...
import pl.lasota.tool.orm.field.PaginationField;
...
public interface SearchService<READING> {
    ...
    default Page<READING> find(List<Field<?>> source) {
        ...
        PaginationField paginationField = first.orElse(new PaginationField(new pl.lasota.tool.orm.field.Pageable(0, 10)));

        pl.lasota.tool.orm.field.Pageable pageable = paginationField.getValue();
